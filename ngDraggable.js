...
angular
    ...
    function($rootScope, $parse, $document, $window, ngDraggable) {
      return {
        ...
        link: function(scope, element, attrs) {
          ...
          var allowTransform = angular.isDefined(attrs.allowTransform)
            ? scope.$eval(attrs.allowTransform)
            : true;
          ...
          var initialize = function() {
            ...
            if (element[0].querySelectorAll) {
              var dragHandles = angular.element(
                element[0].querySelectorAll('[ng-drag-handle]')
              );
            } else {
          ...
          var isClickableElement = function(evt) {
            return angular.isDefined(
              angular.element(evt.target).attr('ng-cancel-drag')
            );
          };
          /*
          ...
          var onlongpress = function(evt) {
            ...
            if (allowTransform) _dragOffset = offset;
            else {
              _dragOffset = {
            ...
            _deregisterRootMoveListener = $rootScope.$on(
              'draggable:_triggerHandlerMove',
              function(event, origEvent) {
                onmove(origEvent);
              }
            );
          };
          ...
          var reset = function() {
            ...
            else element.css({ position: '', top: '', left: '' });
          };
  ...
  .directive('ngDrop', [
    ...
    'ngDraggable',
    'ngDragHitTest',
    function($parse, $timeout, $window, $document, ngDraggable, ngDragHitTest) {
      return {
        restrict: 'A',
        link: function(scope, element, attrs) {
  ...
  .directive('ngDragClone', [
    ...
    '$timeout',
    'ngDraggable',
    function($parse, $timeout, ngDraggable) {
      return {
        restrict: 'A',
        link: function(scope, element, attrs) {
          var img,
          ...
          var preventContextMenu = function() {
            ...
            img.off(
              'mousedown touchstart touchmove touchend touchcancel',
              absorbEvent_
            );
          ...
          var createInterval = function() {
            ...
            nextFrame(function() {
              ...
              if (config.scrollElements) {
                ...
                angular.forEach(config.scrollElements, function(testElement) {
                  //Generate the hittest for each element
                  var dragHitTestResult = ngDragHitTest(
                    testElement,
                    lastDragObj.x,
                    lastDragObj.y,
                    sides
                  ...
                  if (dragHitTestResult.inside) {
                    hoverElements.push({
                      element: testElement,
                      hitTestResult: dragHitTestResult
                    });
                  }
                });
              }
              ...
              angular.forEach(hoverElements, function(hoverElem) {
                ...
                if (config.horizontalScroll) {
                  if (isWindow) {
                    ...
                    var scrollXEnd = Math.max(
                      document.documentElement.clientWidth,
                      window.innerWidth || 0
                    );
                    if (
                      clientX < config.activationDistance // If the mouse is on the left of the viewport within the activation distance.
                    )
                      scrollX = -config.scrollDistance;
                    else if (
                      clientX >
                      scrollXEnd - config.activationDistance // If the mouse is on the right of the viewport within the activation distance.
                    )
                      scrollX = config.scrollDistance;
                  } else if (hoverElem.hitTestResult.right) {
                    //It's an element and it's on its right edge
                    scrollX = config.scrollDistance;
                  } else if (hoverElem.hitTestResult.left) {
                    //It's an element and it's on its left edge
                    scrollX = -config.scrollDistance;
                  }
                }
                ...
                if (config.verticalScroll) {
                  if (isWindow) {
                    ...
                    if (clientY < config.activationDistance) {
                      ...
                      clientY >
                      scrollYEnd - config.activationDistance
                    ) {
                      // If the mouse is on the bottom of the viewport within the activation distance.
                      scrollY = config.scrollDistance;
                    }
                  } else if (hoverElem.hitTestResult.top) {
                    //It's an element and it's on its right edge
                    scrollY = -config.scrollDistance;
                  } else if (hoverElem.hitTestResult.bottom) {
                    //It's an element and it's on its left edge
                    scrollY = config.scrollDistance;
                  }
                }
                ...
                if (scrollX !== 0 || scrollY !== 0) {
                  ...
                  if (isWindow) {
                    ...
                    var horizontalScrollAmount =
                      ($window.pageXOffset ||
                        $document[0].documentElement.scrollLeft) -
                      currentScrollLeft;
                    var verticalScrollAmount =
                      ($window.pageYOffset ||
                        $document[0].documentElement.scrollTop) -
                      currentScrollTop;
                    ...
                    elementToMove[0].scrollTop =
                      elementToMove[0].scrollTop + scrollY;
                    elementToMove[0].scrollLeft =
                      elementToMove[0].scrollLeft + scrollX;
                  }
